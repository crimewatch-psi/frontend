...
const renderCustomizedLabel = ({ name, percent = 0 }: PieLabelRenderProps) => {
...
export default function CrimeStatisticPage({
  ...
  const { categoryData, monthlyData, crimeRateData } = useMemo(() => {
    ...
    crimeData.forEach((crime) => {
      ...
      crimeRateCount[crime.crimeRate] =
        (crimeRateCount[crime.crimeRate] || 0) + 1;
    });
    ...
    return {
      ...
      crimeRateData: Object.entries(crimeRateCount).map(([name, value]) => ({
        name,
        value,
      })),
    };
